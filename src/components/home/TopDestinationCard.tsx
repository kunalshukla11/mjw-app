...
import {
  getSlugFromDashboardData,
  getDestinationQueryParams,
} from '@/src/lib/services/common/slugService';
...
export default function TopDestinationCard({
  ...
  const destinationSlug = getSlugFromDashboardData(dashboadElement);
  const queryParams = getDestinationQueryParams(dashboadElement);

  const destinationUrl = `/holidays/${destinationSlug}${queryParams ? `?${queryParams}` : ''}`;
